<template>
	<div class="data-management">
		...
		<div v-show="activeTab === 'panorama'">
			...
			<el-card class="table-card">
				...
				>
					...
					<el-table-column label="数据ID" prop="id" width="100" />
					<el-table-column label="缩略图" width="100">
						<template #default="{ row }">
							<el-image
								...
								fit="cover"
								:preview-src-list="[loadImage(row.thumbnail)]"
							/>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="350" fixed="right">
						<template #default="{ row }">
							...
							<el-button 
								v-if="!row.is_used && row.status === 'published'" 
								link type="success" 
								size="small" 
			...
			<el-card class="toolbar-card">
				<div class="toolbar">
					<div class="toolbar-left">
						...
						<el-button 
							type="danger" 
							@click="handleBatchDeleteLocations"
					...
					<el-table-column label="操作" width="300" fixed="right">
						<template #default="{ row }">
							<el-button 
								...
								link type="success" 
								size="small" 
								@click="showAssignPanoramaDialog(row)"
							>
								关联全景图
							</el-button>
							<el-button 
								...
								link type="warning" 
								size="small" 
								@click="handleDetachPanorama(row)"
							>
								解除关联
							</el-button>
							...
							<el-button link type="primary" size="small" @click="handleViewLocationDetail(row)">
								查看
							</el-button>
							<el-button 
								link 
		...
		<div v-show="activeTab === 'panoramaPreview'">
			<el-card class="toolbar-card">
				<div class="toolbar">
					<div class="toolbar-left">
						...
						<el-select 
							v-model="previewFilterType" 
							placeholder="筛选类型" 
							style="width: 150px; margin-left: 10px"
							@change="filterPanoramaPreviews"
						>
			...
			<el-card class="preview-container-card">
				<el-row :gutter="20" v-if="panoramaPreviews.length > 0">
					<el-col 
						v-for="item in currentPagePanoramas" 
						:key="item.id" 
						:xs="24" 
						:sm="12" 
						:md="8" 
						:lg="6" 
						class="preview-item-col"
					>
							...
							<div class="preview-thumbnails" v-if="item.preview_images && item.preview_images.length > 0">
								<div class="thumbnails-title">
									...
									<el-button 
										...
										type="primary" 
										size="small" 
										@click.stop="handleAddPreviewForPanorama(item)"
								...
								<div class="thumbnail-list">
									...
									<div 
										v-if="item.preview_images.length > 4" 
							...
							<div class="preview-info">
								...
								<div class="action-buttons">
									<el-button 
										type="primary" 
										size="small" 
										@click="viewPanoramaPreview(item)"
										plain
									>
										查看全景
									</el-button>
									<el-button 
										v-if="!item.is_used && item.status === 'published'"
										type="success" 
										size="small" 
									...
									<el-button 
										type="info" 
										size="small" 
										@click="handleViewPanoramaDetail(item)"
									>
										详情
									</el-button>
									<el-button 
										type="primary" 
										size="small" 
										@click="handleAddPreviewForPanorama(item)"
										plain
									>
										添加预览图
									</el-button>
		...
		<el-dialog v-model="reviewDialogVisible" title="数据审核" width="80%">
			<div v-if="reviewData" class="review-content">
				<div class="review-image">
					<el-image
						:src="loadImage(reviewData.panoramaImage)"
				...
				<div class="detail-info">
					<el-descriptions :column="2" border>
						...
						<el-descriptions-item label="使用地点">
							<template v-if="detailData.location_id">
								{{ detailData.location }} (ID: {{ detailData.location_id }})
							</template>
							<el-tag v-else size="small" type="info">未使用</el-tag>
						</el-descriptions-item>
		...
		<el-dialog v-model="uploadDialogVisible" title="上传新数据" width="600px">
			<el-form :model="uploadForm" label-width="100px" :rules="uploadRules" ref="uploadFormRef">
				...
				<el-form-item label="关联地点（可选）">
					<el-select
						v-model="uploadForm.location_id"
						placeholder="选择要关联的地点"
						style="width: 100%"
						clearable
						filterable
					>
			...
			<template #footer>
				...
				<el-button type="primary" @click="handleConfirmUpload" :loading="uploadLoading">
					确定上传
				</el-button>
			</template>
		</el-dialog>
		...
		<el-dialog 
			...
			:title="locationDialogType === 'add' ? '添加新地点' : '编辑地点'" 
			width="600px"
		>
			<el-form 
				:model="locationForm" 
		...
		<div v-if="assignData" class="assign-content">
			...
			<div style="margin-top: 20px;">
				...
				>
				<el-option
					v-for="loc in assignData.availableLocations"
					:key="loc.id"
		...
		<!-- 添加预览图对话框 -->
		<el-dialog 
			v-model="addPreviewDialogVisible" 
			:title="`为全景图 ${currentPanorama?.id || ''} 添加预览图`" 
			width="600px"
		>
					...
					>
						...
						<template #tip>
							<div class="el-upload__tip">
								支持jpg/png格式文件，且不超过10MB，最多可上传10张图片
							</div>
						</template>
					</el-upload>
			...
			<template #footer>
				...
				<el-button 
					type="primary" 
					@click="handleConfirmAddPreview"
					:disabled="addPreviewFiles.length === 0"
		...
		<el-dialog v-model="galleryDialogVisible" title="预览图画廊" width="90%" fullscreen>
			<div v-if="currentGalleryData" class="gallery-container">
				<div class="gallery-main">
					<el-image
						:src="loadImage(currentGalleryData.currentImage)"
				...
				<div class="gallery-controls">
					...
					<div class="gallery-nav">
						<el-button 
							:disabled="currentGalleryData.currentIndex === 0"
    ...
    if (response.code === "200" && response.data) {
      ...
      if (locationFilter.value) {
        filteredData = filteredData.filter((loc: any) => 
          loc.category === locationFilter.value
        );
      }
...
const prevGalleryImage = () => {
  if (currentGalleryData.value && currentGalleryData.value.currentIndex > 0) {
    ...
    currentGalleryData.value.currentImage = 
      currentGalleryData.value.images[currentGalleryData.value.currentIndex];
  }
};

const nextGalleryImage = () => {
  if (currentGalleryData.value && 
  ...
  try {
    ...
    if (response.code === "200" && response.data) {
      ...
      tableData.value = filteredData.slice(start, end).map((item: any) => ({
        ...
        location_name: item.location_name,
        status: item.status,
        statusText: item.status === 'published' ? '已发布' : 
...
const handleBatchDeleteLocations = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedLocations.value.length} 个地点吗？`,
      "批量删除地点",
      {
